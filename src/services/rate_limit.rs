...
impl RateLimitService {
    ...
    pub async fn check_file_upload_rate_limit(&self, user_id: Uuid) -> Result<(), String> {
        let mut limits = self.file_upload_limits.write().await;
        let now = Instant::now();
        
        let entry = limits.entry(user_id).or_insert(FileUploadRateLimit {
            count: 0,
            window_start: now,
        });
        ...
        if entry.count >= self.config.file_uploads_per_hour {
            return Err(format!(
                "Rate limit exceeded. Maximum {} file uploads per hour.",
                self.config.file_uploads_per_hour
            ));
        }
    ...
    pub async fn check_registration_rate_limit(&self, ip: IpAddr) -> Result<(), String> {
        let mut limits = self.registration_limits.write().await;
        let now = Instant::now();
        
        let entry = limits.entry(ip).or_insert(RegistrationRateLimit {
            count: 0,
            window_start: now,
        });
        ...
        if entry.count >= self.config.registration_attempts_per_hour {
            return Err(format!(
                "Rate limit exceeded. Maximum {} registration attempts per hour.",
                self.config.registration_attempts_per_hour
            ));
        }
    ...
    /// Check if IP can attempt login
    pub async fn check_login_rate_limit(&self, ip: IpAddr) -> Result<(), String> {
        let mut limits = self.login_limits.write().await;
        let now = Instant::now();
        
        let entry = limits.entry(ip).or_insert(LoginRateLimit {
            count: 0,
            window_start: now,
        });
        ...
        if entry.count >= self.config.login_attempts_per_minute {
            return Err(format!(
                "Rate limit exceeded. Maximum {} login attempts per minute.",
                self.config.login_attempts_per_minute
            ));
        }
    ...
    pub async fn get_stats(&self) -> RateLimitStats {
        let message_count = self.message_limits.read().await.len();
        let request_count = self.request_limits.read().await.len();
        let file_upload_count = self.file_upload_limits.read().await.len();
        let registration_count = self.registration_limits.read().await.len();
        let login_count = self.login_limits.read().await.len();
        
        RateLimitStats {
            tracked_users_messages: message_count,
            tracked_ips_requests: request_count,
            tracked_users_uploads: file_upload_count,
            tracked_ips_registrations: registration_count,
            tracked_ips_logins: login_count,
        }
    }
}
